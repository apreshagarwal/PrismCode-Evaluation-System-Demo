...
$quiz_id = $_GET['id'];
$sql = "SELECT * FROM `quizes` WHERE `quiz_id` = '$quiz_id' LIMIT 1";
...
$exam_name = $quiz_details['quiz_name'];
$difficulty_level = $quiz_details['difficulty_level'];
$description = $quiz_details['description'];
$number_of_questions = $quiz_details['number_of_questions'];
$is_active = $quiz_details['is_active'];
?>
<!doctype html>
...
<head>
    ...
    <title>Edit Exam | Online Evaluation System | PrismCode Info Solutions Pvt Ltd</title>
    ...
    <div class="page">
        <div class="page-main">
            ...
            if (isset($_POST['submit']) && isset($_POST['exam_name']) && isset($_POST['difficulty_level']) && isset($_POST['description']) && isset($_POST['number_of_questions']) && isset($_POST['is_active']) && ($quiz_id != 1)) {  // ID 1 refers to the PHP Demo Quiz
                ...
                $sql = "UPDATE `quizes` SET `quiz_name` = '$exam_name', `difficulty_level` = '$difficulty_level', `description` = '$description', `number_of_questions` = '$number_of_questions', `is_active` = '$is_active' WHERE `quiz_id` = '$quiz_id'";
                if ($conn->query($sql)) {
            ?>
                    <div class="alert alert-icon alert-success alert-dismissible" role="alert">
                        <button type="button" class="close" data-dismiss="alert"></button>
                        <i class="fe fe-check mr-2" aria-hidden="true"></i> The exam <b><?php echo $exam_name ?></b> has been updated successfully.
                    </div>
                <?php
                } else {
                ?>
                    <div class="alert alert-icon alert-danger alert-dismissible" role="alert">
                        <button type="button" class="close" data-dismiss="alert"></button>
                        <i class="fe fe-alert-triangle mr-2" aria-hidden="true"></i> Exam updation failed. Please try again later.
                    </div>
            <?php
                }
            }
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <h3 class="card-title">Edit <?php echo $exam_name; ?></h3>
                                </div>
                                <div class="card-body">
                                    <form id="update_exam_form" action="#" method="post">
                                        <div class="row">
                                            ...
                                            <div class="col-sm-12 col-lg-12">
                                                <div class="form-group">
                                                    ...
                                                    <div class="selectgroup w-100">
                                                        <label class="selectgroup-item">
                                                            <input type="radio" name="is_active" value="1" <?php if ($is_active == "1") echo "checked=\"\""; ?> class="selectgroup-input" required>
                                                            <span class="selectgroup-button">Active</span>
                                                        </label>
                                                        <label class="selectgroup-item">
                                                            <input type="radio" name="is_active" value="0" <?php if ($is_active == "0") echo "checked=\"\""; ?> class="selectgroup-input">
                                ...
                                <div class="card-footer text-right">
                                    <div class="d-flex">
                                        <button form="update_exam_form" type="reset" class="btn btn-secondary">Reset</button>
                                        <button form="update_exam_form" type="submit" name="submit" class="btn btn-primary ml-auto">Update Exam</button>
                                    </div>
                                </div>
                            </div>
                        </div>
